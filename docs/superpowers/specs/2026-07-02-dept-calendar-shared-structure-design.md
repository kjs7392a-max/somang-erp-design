...
### ★ 불변식 (모든 부서·모든 테이블 동일 적용)
**어떤 부서도 타 부서의 데이터를 열람할 수 없다.** 예외 없음(총무 포함).
- 사용자는 **자기 부서로 스코프된 행만** 본다.
- "자기 부서로 스코프" = ① 자기 부서가 소유한 행, 또는 ② 자기 부서로 **배달(routed)된** 행. 둘 다 결국 "내 부서" 기준. 타 부서 행을 직접 조회하는 정책은 어디에도 만들지 않는다.

### 읽기(SELECT) — 지금 활성화
세 테이블 공통: 로그인 사용자가 **`corporation_id = 내 법인 AND department = 내 부서`**인 행만 조회.
- 간호과원 → 간호과 전병동 근무표(+휴가) 조회 (같은 부서 내이므로 OK)
- 원무과원 → 원무과 휴가 조회 (같은 부서 내이므로 OK)
...
### 통합부서(총무과) = 중앙 수신처 (열람이 아니라 "배달 수신")
위 불변식은 총무에도 그대로 적용 — **총무도 타 부서 행을 직접 열람하지 않는다.** 총무가 데이터를 받는 방식은 **연동 부서가 상신/전송하면 그 데이터가 총무에게 배달(routed)된 레코드**를 총무가 읽는 것. 즉 총무는 언제나 "총무로 스코프된 행"만 본다. dash-erp의 "법인별(생산자)→통합(수신·처리)" 흐름.
- 총무의 데이터 범위 = "총무에게 배달된 것"이지 "아무 부서나 조회"가 아니다.
- 총무 소속은 SM이지만 수신은 **SM+HD 양 법인** 모두. 수신 데이터는 `corporation_id` 태그로 법인별 필터.
- **배달(routing) 구현은 대시보드 Phase.** 방식(예: 수신 레코드 생성 / `recipient_dept` 태그 / 별도 inbox)은 그때 확정하되, **어떤 경우에도 "총무=타부서 read-all" 정책은 만들지 않는다.**
- 모바일에서 총무과원은 `personal` variant(개인 일정)만 본다. 이번 스펙은 배달 수신을 구현하지 않되, 스키마가 이를 지원(모든 행 `corporation_id` 태그, 승인 트리거가 승인건을 `leaves`로 흘려보냄)하도록만 설계.
...
- **불변식(§5): 모든 부서가 자기 부서 행만 열람. 어떤 부서도 타 부서를 못 봄(총무 포함).** 총무의 수신은 "타부서 열람"이 아니라 **배달된 레코드** 읽기. 배달(routing) 방식은 대시보드 Phase에서 확정하되 read-all 정책은 금지. 이번 스키마는 `corporation_id` 태그 + 승인 트리거로 지원.
