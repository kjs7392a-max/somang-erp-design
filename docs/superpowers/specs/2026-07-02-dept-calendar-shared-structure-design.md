...
### 통합부서(총무과) = 중앙 수신처 (열람이 아니라 "수신")
총무과는 타 부서를 뒤져보는(browse/read-all) 권한을 갖지 않는다. 대신 **연동 부서(생산자)가 상신·전송한 데이터를 각 법인(SM+HD) 걸쳐 수신**한다(directed delivery). dash-erp의 "법인별(생산자)→통합(수신·처리)" 흐름.
- 즉 총무의 데이터 범위 = "나에게 전송된/승인 완료된 것"이지 "아무 부서나 조회"가 아니다.
- 총무 소속은 SM이지만 수신은 **SM+HD 양 법인** 모두. 수신 데이터는 `corporation_id` 태그로 법인별 필터.
- 이 수신 로직은 **대시보드 Phase**의 기능. 모바일에서 총무과원은 `personal` variant(개인 일정)만 본다. 이번 스펙은 수신을 구현하지 않되, **스키마가 이를 지원**하도록 설계(모든 행이 `corporation_id` 태그 보유, 트리거가 승인건을 `leaves`로 흘려보냄 → 대시보드가 수신처로 구독).
...
- 통합부서(총무과)는 **타 부서 열람이 아니라 수신처**(§5 참조). 연동 부서가 상신한/승인된 데이터를 SM+HD 걸쳐 수신. 수신 구현은 대시보드 Phase. 이번 스키마는 `corporation_id` 태그 + 승인 트리거로 이 수신을 지원하도록 설계.
