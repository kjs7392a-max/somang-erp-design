...
import { isInAppBrowser, getMobileOS } from "@/lib/in-app-browser";
...
export default function MypagePage() {
  ...
  const handleRegisterBiometric = async () => {
    ...
    const targetBrowser = getMobileOS() === "ios" ? "Safari" : "Chrome";
    if (isInAppBrowser()) {
      alert(
        `카카오톡 등 인앱 브라우저에서는 지문 등록이 안 됩니다.\n` +
          `우측 상단 메뉴에서 '${targetBrowser}(외부 브라우저)으로 열기' 후 다시 시도해주세요.`,
      );
      return;
    }
    if (!isWebAuthnSupported()) {
      alert(
        `이 브라우저는 지문 로그인을 지원하지 않습니다.\n` +
          `${targetBrowser}(으)로 열거나 홈 화면에 설치 후 이용해주세요.`,
      );
      return;
    }
    setBioLoading(true);
