...
export function ApprovalStatusCard({ onGoList }: { onGoList?: () => void }) {
  ...
  return (
    <AccordionCard
      title={t("approval_card_title")}
      icon={<ClipboardCheck className="h-5 w-5 text-[#3b5bdb]" strokeWidth={2.2} />}
      summary={
        loading ? (
          <span className="text-xs text-zinc-400">{t("loading")}</span>
        ) : (
          <span className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs text-zinc-500">
            <span>
              {t("approval_waiting")} <strong className="text-[#e03131]">{items.length}</strong>
            </span>
            {urgentItems.length > 0 && (
              <>
                ...
                <span className="inline-flex items-center gap-0.5">
                  ...
                  <span className="text-red-600 font-semibold">{t("approval_urgent")} {urgentItems.length}</span>
                  <span className="max-w-[140px] truncate text-zinc-500">· {urgentItems[0].title}</span>
                </span>
              </>
            )}
          </span>
    ...
    >
      <div className="space-y-2.5">
        {loading ? (
          ...
          <p className="py-3 text-center text-sm text-zinc-400">{t("approval_card_empty")}</p>
        ) : (
          items.map((item) => (
  ...
  return (
    ...
    >
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-[1.0625rem] font-bold text-zinc-900">{t("approval_card_title")}</h2>
        {urgentCount > 0 && (
          <span className="inline-flex items-center gap-1 rounded-full bg-red-50 px-2 py-0.5 text-xs font-semibold text-red-600">
            <AlertCircle className="h-3 w-3" strokeWidth={2.5} />
            {t("approval_urgent")} {urgentCount}
          </span>
        )}
        <ChevronRight className="h-5 w-5 text-zinc-400" />
      </div>
      {loading ? (
        <p className="py-4 text-center text-sm text-zinc-400">{t("loading")}</p>
      ) : (
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col gap-2.5">
            <div className="flex items-center">
              ...
              <span className="flex-1 text-sm text-zinc-600">{t("approval_waiting")}</span>
