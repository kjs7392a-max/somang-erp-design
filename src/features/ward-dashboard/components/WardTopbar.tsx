...
import type { WardAccount } from "@/features/ward-dashboard/types";
import { ACCOUNTS } from "@/features/ward-dashboard/data";
...
interface Props {
  ...
  user: WardAccount;
  today: string;
  onSwitch: (acc: WardAccount) => void;
}

export function WardTopbar({ title, crumbs, user, today, onSwitch }: Props) {
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
        ...
        <span style={{ position: "relative", cursor: "pointer", color: C.textMuted }}>
          ...
          <span style={{ position: "absolute", top: -3, right: -4, width: 15, height: 15, borderRadius: "50%", background: C.danger, color: "#fff", fontSize: 9, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>4</span>
        </span>
                  ...
                  return (
                    <div key={a.id} onClick={() => { onSwitch(a); setOpen(false); }}
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 10.5, color: C.textFaint }}>{a.ward.name}</div>
                      </div>
                      {a.canApprove
